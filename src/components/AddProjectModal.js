...
const AddProjectModal = ({ isModalOpen, closeModal, edit = false, id = null }) => {
    ...
    useEffect(() => {
        if (edit && isModalOpen) {
            axios.get(`https://capstone-project-management-be.onrender.com/project/${id}`)
                .then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!edit) {
            axios.post('https://capstone-project-management-be.onrender.com/project/', { title, description: desc },{
            ...
            axios.put(`https://capstone-project-management-be.onrender.com/project/${id}`, { title, description: desc })
                .then((res) => {
                    closeModal()
                    const customEvent = new CustomEvent('projectUpdate', { detail: { ...res.data } });
                    document.dispatchEvent(customEvent);
                        ...
                        >
                            <Dialog.Panel className="rounded-md bg-white w-6/12">
                                ...
                                <Dialog.Title as='div' className={'bg-white shadow px-6 py-4 rounded-t-md sticky top-0'}>
                                    ...
                                    <button onClick={() => closeModal()} className=' absolute right-6 top-4 text-gray-500 hover:bg-gray-100 rounded focus:outline-none focus:ring focus:ring-offset-1 focus:ring-black'>
